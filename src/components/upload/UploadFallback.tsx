...
export function UploadFallback({ orderId }: { orderId: string }) {
 ...
 return (
  <section className="w-full flex flex-col gap-6">
   <div className="bg-card/40 backdrop-blur-xl border border-border/50 rounded-card p-8 md:p-10 flex flex-col relative overflow-hidden shadow-2xl">
    <div className="absolute top-0 right-0 w-40 h-40 bg-primary/5 rounded-bl-full -z-10 blur-3xl" />
    
    <div className="flex items-center gap-4 mb-6">
     <div className="w-12 h-12 bg-primary/10 rounded-card flex items-center justify-center text-primary border border-primary/20">
      <Link2 size={24} />
     </div>
    ...
    <div className="flex flex-wrap gap-3 mt-auto relative z-10">
     <button 
      ...
      className="w-fit flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-button bg-card/50 backdrop-blur-sm border border-border shadow-sm hover:bg-muted hover:border-primary/20 transition-all group text-sm font-semibold"
     >
      {copied ? (
